"""Regression tests: no RuntimeWarnings from public API methods on real data.

Each test runs a single public method with real fixture data and asserts that
no RuntimeWarning is emitted. Methods that currently do warn are marked
xfail so they don't silently regress; resolve the underlying issue and
remove the marker when fixed.
...
class TestNoWarningsPreprocessing:
