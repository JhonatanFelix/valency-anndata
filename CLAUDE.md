...
When adding a new public function to any module (`val.pp`, `val.tl`, `val.viz`, `val.datasets`), always update all of the following:

1. The module's `__init__.py` — import and add to `__all__`
2. The corresponding `docs/api/<module>.md` — add a `### ::: valency_anndata.<module>.<function>` entry
